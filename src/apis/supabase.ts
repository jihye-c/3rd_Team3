...
export default class Supabase {
  ...
  static async addCSVDataTraffic(file: File) {
    ...
    console.log('전체 rows : ', hospitals.length);
    // 병원 ID Set 생성
    const validHospitalIds = new Set(
      hospitals?.map((hospital: {id: string}) => String(hospital.id)),
    );
    console.log('ID 목록 :', validHospitalIds);
        ...
        Papa.parse(csvData, {
          ...
          complete: async (results) => {
            ...
            for (let i = 0; i < records.length; i += batchSize) {
              ...
              const {data, error} = await supabase.from('hospital_traffic').upsert(batch);
    ...
    reader.readAsText(file); // FileReader 실행 추가
  }
  static async addCSVDataDetail(file: File) {
    const supabase = this.init();
    if (!supabase) return;
        ...
        Papa.parse(csvData, {
          ...
          complete: async (results) => {
            ...
            for (let i = 0; i < records.length; i += batchSize) {
              ...
              const {data, error} = await supabase.from('hospital_detail').upsert(batch);
  ...
  static async addCSVData2(file: File) {
    const supabase = this.init();
    if (!supabase) return;
    const reader = new FileReader();
        ...
        Papa.parse(csvData, {
          ...
          complete: async (results) => {
            ...
            for (let i = 0; i < records.length; i += batchSize) {
              ...
              const {data, error} = await supabase.from('hospital').upsert(batch);
